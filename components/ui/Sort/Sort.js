	...
	return (
		<div ref={sortRef} className={styles.Sort}>
			<div className={styles.SortLabel}>
				<svg
					className={classNames(styles.SortArrow, activeArrow && styles.SortArrowActive)}
					width="10"
					height="6"
					viewBox="0 0 10 6"
					fill="none"
				...
				<span onClick={() => onClickButton()} className={classNames(styles.SortText, styles.SortButton)}>
					{sortActive.name}
				</span>
			</div>
			{openPopup && (
						...
						{sortList.map((objSort, index) => (
							<li
								className={classNames(
									...
									sortActive.sortProp === objSort.sortProp ? styles.SortPopupItemActive : null
								)}
								key={index}
